...
metadata = json.loads(
    subprocess.check_output(
        [
            ...
            "--format-version=1",
            "--manifest-path",
            f"{home_dir}/tock/boards/tutorials/nrf52840dk-thread-tutorial/Cargo.toml",
        ]
    )
)
...
folders = ["nrf52840dk-thread-tutorial"]
...
for folder in folders:
    ...
    for crate in result:
        ...
        sorted_files.extend(
            [
                f.strip()
                for f in open("tmp/llvm_files_tock.txt")
    ...
    command = [
        f"{home_dir}/.rustup/toolchains/nightly-2025-03-14-x86_64-unknown-linux-gnu/lib/rustlib/x86_64-unknown-linux-gnu/bin/llvm-link",
        "--only-needed",
        *sorted_files,
        "-o",
        output_file,
    ...
    command = [
        f"{home_dir}/.rustup/toolchains/nightly-2025-03-14-x86_64-unknown-linux-gnu/lib/rustlib/x86_64-unknown-linux-gnu/bin/llvm-dis",
        output_file,
        "-o",
        final_file,
    ]
